...
import { Order, UserProfile, Product, Subscription, Redemption, Feedback } from '../types';
import { t } from '../i18n';
import { PRODUCTS } from '../constants';
import { motion, AnimatePresence } from 'motion/react';
import { Package, Clock, CheckCircle, Truck, X, User as UserIcon, Users, Settings, LogOut, LayoutDashboard, ShoppingBag, Plus, Trash2, Edit2, Save, Search, Filter, Download, Upload, FileSpreadsheet, FileText, AlertCircle, CheckCircle2, Calendar, RefreshCw, Gift, Tag, Sparkles, MessageSquare } from 'lucide-react';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ user, onLogout, onClose }) => {
  const [activeTab, setActiveTab] = useState<'orders' | 'users' | 'products' | 'settings' | 'subscriptions' | 'redemptions' | 'feedbacks'>('orders');
  ...
  useEffect(() => {
    ...
    const unsubFeedbacks = onSnapshot(query(collection(db, 'feedbacks'), orderBy('createdAt', 'desc')), (snapshot) => {
      ...
      {orderToDelete && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4">
          <div className="bg-white p-8 rounded-[32px] shadow-2xl space-y-6 max-w-sm w-full border border-primary/10">
            <h2 className="text-2xl font-bold text-primary">{t(user.language || 'en', 'confirmDelete')}</h2>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <main className="lg:col-span-3 space-y-8">
            ...
            {activeTab === 'orders' ? (
                      ...
                      <div className="flex flex-col sm:flex-row justify-between items-start gap-6 mb-8">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="font-bold text-xl text-primary">{order.customerName || users.find(u => u.uid === order.userId)?.displayName || 'Unknown Customer'}</p>
              ...
              <div className="space-y-6">
                <h2 className="text-2xl font-bold text-primary mb-6">Active Subscriptions</h2>
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-start gap-6 mb-6">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <p className="font-bold text-xl text-primary">{users.find(u => u.uid === sub.userId)?.displayName || 'Unknown Customer'}</p>
              ...
              <div className="space-y-6">
                <h2 className="text-2xl font-bold text-primary mb-6">Loyalty Redemptions</h2>
                        ...
                        <div className="flex flex-col sm:flex-row justify-between items-start gap-6 mb-6">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              ...
                              <p className="font-bold text-xl text-primary">{redemption.userName}</p>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 gap-6">
                  {feedbacks.map((fb) => (
                    <div key={fb.id} className="glassmorphism p-8 rounded-[32px] border border-white/10 shadow-xl relative group">
                      <div className="flex justify-between items-start mb-6">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <p className="text-[10px] font-bold text-primary/60 mb-1 uppercase tracking-[0.2em]">Feedback ID: {fb.id}</p>
                            <h3 className="text-xl font-bold text-primary">{fb.userName}</h3>
                            <p className="text-xs text-text/60 font-medium">{fb.userPhone} • {fb.location}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="text-[10px] font-bold text-primary/40 uppercase tracking-widest">{fb.date}</p>
                          <p className="text-[10px] font-bold text-primary/40 uppercase tracking-widest">{fb.time}</p>
                        </div>
                      </div>
                      <div className="bg-white/5 rounded-2xl p-6 border border-white/10">
                        <p className="text-sm font-medium leading-relaxed italic text-text/80">"{fb.message}"</p>
                      </div>
                      <button 
                        onClick={() => deleteFeedback(fb.id!)}
                        className="absolute bottom-4 right-4 p-3 text-red-400 hover:text-red-500 hover:bg-red-500/10 rounded-xl transition-all opacity-0 group-hover:opacity-100 border border-transparent hover:border-red-500/20"
                      >
                        <Trash2 size={18} />
                      </button>
                    </div>
                  ...
                  {feedbacks.length === 0 && (
                    <div className="text-center py-20 bg-white/5 border border-dashed border-white/10 rounded-[32px]">
                      <MessageSquare size={48} className="mx-auto mb-4 text-primary/10" />
                      <p className="text-text/40 font-bold uppercase tracking-widest text-[10px]">No feedbacks received yet</p>
                    </div>
                  )}
                </div>
              </div>
                ...
                <div className="pt-10 border-t border-white/10 space-y-8">
                  ...
                  <div className="bg-primary/5 p-6 rounded-2xl border border-primary/10">
                    <p className="text-sm text-primary font-medium">
                      Lyra AI is now powered by <strong>OpenAI</strong>. 
                    </p>
                    <p className="text-[10px] text-primary/60 mt-2 uppercase tracking-widest font-bold">
